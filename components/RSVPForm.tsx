...
// NOVO: onConfirm agora avisa se deu tudo certo ou não (Promise<boolean>)
interface RSVPFormProps {
  onConfirm: (guest: Guest) => Promise<boolean>;
}
...
export const RSVPForm: React.FC<RSVPFormProps> = ({ onConfirm }) => {
  ...
  // NOVO: Adicionamos o 'async' aqui pois vamos se comunicar com o banco de dados
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // NOVO: Esperamos (await) a resposta do App.tsx para saber se salvou ou se era duplicado
    const isSuccess = await onConfirm(newGuest);
    
    if (!isSuccess) {
      alert(`O nome "${name}" já consta na nossa lista de confirmados!\n\nSe for você, vamos te redirecionar para a página de presentes.`);
    }
    
    // De qualquer forma (sucesso ou duplicado), nós criamos a "trava" no celular da pessoa
  ...
  if (hasConfirmed) {
    return (
      <div className="bg-green-50 p-10 rounded-[2.5rem] shadow-2xl border border-green-100 h-full flex flex-col items-center justify-center text-center animate-in fade-in duration-700">
        ...
        <a 
          href="https://listapresente-brown.vercel.app" 
          target="_blank"
          rel="noopener noreferrer"
          className="bg-green-600 hover:bg-green-700 text-white font-black py-4 px-8 rounded-2xl shadow-lg transform transition-all active:scale-95 mb-8 tracking-wide uppercase text-sm inline-block"
        >
